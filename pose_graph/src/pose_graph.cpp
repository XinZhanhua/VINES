...
void PoseGraph::addKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop)
{
	...
	if (loop_index != -1)
	{
        ...
        int ifloop = cur_kf->findConnection(old_kf);
        // ROS_INFO("loop_index = %d ifloop = %d", loop_index, ifloop);
...
int PoseGraph::detectLoop(KeyFrame* keyframe, int frame_index)
{
    ...
    if (DEBUG_IMAGE)
    {
        ...
        if (ret.size() > 0)
            putText(loop_result, "neighbour score:" + to_string(ret[0].Score), cv::Point2f(10, 50), FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255));
    }
    // visual loop result 
    if (DEBUG_IMAGE)
    {
        for (unsigned int i = 0; i < ret.size(); i++)
        {
            ...
            putText(tmp_image, "index:  " + to_string(tmp_index) + "loop score:" + to_string(ret[i].Score), cv::Point2f(10, 50), FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255));
        ...
        for (unsigned int i = 1; i < ret.size(); i++)
        {
            ...
            {          
                ...
                if (DEBUG_IMAGE && 0)
                {
                    auto it = image_pool.find(tmp_index);
                    cv::Mat tmp_image = (it->second).clone();
                    putText(tmp_image, "loop score:" + to_string(ret[i].Score), cv::Point2f(10, 50), FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar(255));
                    cv::hconcat(loop_result, tmp_image, loop_result);
                }
            }
