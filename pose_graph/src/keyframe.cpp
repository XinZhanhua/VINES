...
bool KeyFrame::findConnection(KeyFrame* old_kf)
{
	...
	matched_id = point_id;
	// ROS_INFO("matched_2d_cur.size()(beforebefore) = %d", (int)matched_2d_cur.size());
	...
	*/
	...
	double relative_yaw;
	// ROS_INFO("matched_2d_cur.size()(before) = %d", (int)matched_2d_cur.size());
	if ((int)matched_2d_cur.size() > MIN_LOOP_NUM)
	{
	    ...
	    reduceVector(matched_id, status);
	    	...
	    	if (DEBUG_IMAGE)
	        {
	            ...
	            putText(notation, "current frame: " + to_string(index) + "  sequence: " + to_string(sequence), cv::Point2f(20, 30), FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(255), 3);

	            putText(notation, "previous frame: " + to_string(old_kf->index) + "  sequence: " + to_string(old_kf->sequence), cv::Point2f(20 + COL + gap, 30), FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(255), 3);
    ...
    // ROS_INFO("matched_2d_cur.size() = %d", (int)matched_2d_cur.size());
	if ((int)matched_2d_cur.size() > MIN_LOOP_NUM)
	{
