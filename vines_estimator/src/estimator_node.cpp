...
queue<std_msgs::HeaderConstPtr> encoder_buf;
...
void encoder_callback(const std_msgs::Header::ConstPtr& msg)
{
    ...
    if (estimator.encoder_data < 1000 && estimator.last_encoder_data > 3000)
        estimator.rot_times++;
    estimator.continuous_encoder_data = estimator.rot_times * 4096 + estimator.encoder_data;
    // encoder_buf.push(msg);
    if (fabs(estimator.last_continuous_encoder_data - estimator.continuous_encoder_data) > 2000)
...
{
    ros::init(argc, argv, "vines_estimator");
    ros::NodeHandle n("~");
