...
void pubOdometry(const Estimator &estimator, const std_msgs::Header &header)
{
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        ...
        foutC << estimator.Ps[WINDOW_SIZE].x() << ","
              ...
              << estimator.Encoder_angle[WINDOW_SIZE][0] << "," << endl;
        foutC.close();
    }
}
...
void pubCameraPose(const Estimator &estimator, const std_msgs::Header &header)
{
    ...
    {
        int i = idx2;
        Vector3d P = estimator.Ps[i] + estimator.Rs[i] * estimator.tic[i];
        Quaterniond R = Quaterniond(estimator.Rs[i] * estimator.ric[i]);
...
void pubPointCloud(const Estimator &estimator, const std_msgs::Header &header)
{
    ...
    for (auto &it_per_id : estimator.f_manager.feature)
    {
        ...
        Vector3d w_pts_i = estimator.Rs[imu_i] * (estimator.ric[imu_i] * pts_i + estimator.tic[imu_i]) + estimator.Ps[imu_i];
...
void pubTF(const Estimator &estimator, const std_msgs::Header &header)
{
    ...
    // camera frame
    transform.setOrigin(tf::Vector3(estimator.tic[WINDOW_SIZE].x(),
                                    estimator.tic[WINDOW_SIZE].y(),
                                    estimator.tic[WINDOW_SIZE].z()));
    q.setW(Quaterniond(estimator.ric[WINDOW_SIZE]).w());
    q.setX(Quaterniond(estimator.ric[WINDOW_SIZE]).x());
    q.setY(Quaterniond(estimator.ric[WINDOW_SIZE]).y());
    q.setZ(Quaterniond(estimator.ric[WINDOW_SIZE]).z());
    transform.setRotation(q);
    ...
    odometry.header = header;
    odometry.header.frame_id = "world";
    odometry.pose.pose.position.x = estimator.tic[WINDOW_SIZE].x();
    odometry.pose.pose.position.y = estimator.tic[WINDOW_SIZE].y();
    odometry.pose.pose.position.z = estimator.tic[WINDOW_SIZE].z();
    Quaterniond tmp_q{estimator.ric[WINDOW_SIZE]};
    odometry.pose.pose.orientation.x = tmp_q.x();
    odometry.pose.pose.orientation.y = tmp_q.y();
    odometry.pose.pose.orientation.z = tmp_q.z();
    odometry.pose.pose.orientation.w = tmp_q.w();
...
void pubKeyframe(const Estimator &estimator)
{
    ...
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR && estimator.marginalization_flag == 0)
    {
        ...
        // ROS_INFO("feature size: %d", estimator.f_manager.feature.size());
        int temp = 0;
        for (auto &it_per_id : estimator.f_manager.feature)
        {
            int frame_size = it_per_id.feature_per_frame.size();
            if(it_per_id.solve_flag == 1)temp++;
            if(it_per_id.start_frame < WINDOW_SIZE - 2 && it_per_id.start_frame + frame_size - 1 >= WINDOW_SIZE - 2 && it_per_id.solve_flag == 1/* || it_per_id.start_frame == 0 && frame_size == 0*/)
            {

                int imu_i = it_per_id.start_frame;
                Vector3d pts_i = it_per_id.feature_per_frame[0].point * it_per_id.estimated_depth;
                Vector3d w_pts_i = estimator.Rs[imu_i] * (estimator.ric[imu_i] * pts_i + estimator.tic[imu_i])
                                      + estimator.Ps[imu_i];
                geometry_msgs::Point32 p;
                p.x = w_pts_i(0);
                p.y = w_pts_i(1);
        ...
        // ROS_INFO("size: %d, temp = %d, pub = %d", estimator.f_manager.feature.size(), temp, point_cloud.points.size());
