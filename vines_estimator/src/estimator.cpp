...
void Estimator::setParameter()
{
    ...
    ProjectionEncoderFactor::sqrt_info = FOCAL_LENGTH / 1.5 * Matrix2d::Identity();
...
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, const std_msgs::Header &header)
{
    ...
    if (f_manager.addFeatureCheckParallax(frame_count, image, td, encoder_angle_velocity, encoder_data))
    ...
    if (encoder_data > 3000 && last_encoder_data < 1000)
        rot_times--;
    if (encoder_data < 1000 && last_encoder_data > 3000)
        rot_times++;
    ...
    Encoder_angle[frame_count][0] = (double)(encoder_data - 2048) / 4096 * 2 * CV_PI;
    Encoder_angle_original[frame_count] = Encoder_angle[frame_count][0];
    if (fabs(encoder_data - last_encoder_data) > 2000)
        encoder_angle_velocity = last_encoder_angle_velocity;
    else
        encoder_angle_velocity = 0.2 * (double)(encoder_data - last_encoder_data) / 4096 * 2 * CV_PI / (header.stamp.toSec() - last_time) + 0.8 * last_encoder_angle_velocity;
    
    last_encoder_data = encoder_data;
    last_encoder_angle_velocity = encoder_angle_velocity;
    ...
    // else
    //     Encoder_angle[frame_count][0] = (double)(4096 - encoder_data) / 4096 * 2 * CV_PI;
...
bool Estimator::initialStructure()
{
    ...
    for (int i = 0; frame_it != all_image_frame.end( ); frame_it++)
    {
        ...
        if((frame_it->first) == Headers[i].stamp.toSec())
        {
            ...
            frame_it->second.R = Q[i].toRotationMatrix() * RIE[0].transpose();
        ...
        cv::cv2eigen(t, T_pnp);
        T_pnp = R_pnp * (-T_pnp);
        frame_it->second.R = R_pnp * RIE[0].transpose();
...
bool Estimator::visualInitialAlign()//VIO联合初始化
{
    TicToc t_g;
    VectorXd x;
    //solve scale
    ...
    tie[0] = TIE[0];
    f_manager.setRic(ric);
    f_manager.triangulate(Ps, &(TIC_TMP[0]), &(RIE[0]), solver_flag);
    ...
    for (int i = frame_count; i >= 0; i--)
        Ps[i] = s * Ps[i] - Rs[i] * (TIE[0] + TEC[0]) - (s * Ps[0] - Rs[0] * (TIE[0] + TEC[0]));
    int kv = -1;
...
void Estimator::solveOdometry()
{
    ...
    {
        TicToc t_tri;
        f_manager.triangulate(Ps, tic, ric, solver_flag);
...
void Estimator::vector2double()
{
    ...
    for (int i = 0; i < NUM_OF_CAM; i++)
    {
        if(ENCODER_ENABLE)
        {
            // ROS_INFO("encoder_data: %d", encoder_data);
            // //这部分代码为，将编码器数据带入
            // Eigen::Matrix4d pose = Eigen::Matrix4d::Identity();
            // pose.block<3, 3>(0, 0) = RIE[i];
            // pose.block<3, 1>(0, 3) = TIE[i];

            // last_encoder_data = encoder_data;
            // double theta = -(4095 - encoder_data) / 4096.0 * 2 * M_PI;
            // Eigen::Matrix4d T = Eigen::Matrix4d::Identity();
            // T.block<3, 3>(0, 0) = Eigen::AngleAxisd(theta + M_PI, Eigen::Vector3d::UnitZ()).toRotationMatrix();
            // T = T * pose;

            // ric[i] = T.block<3, 3>(0, 0);
        ...
        // last_encoder_data = encoder_data;
            //dx dy dz x y z w
        // ROS_INFO("IE: %.3f %.3f %.3f %.2f %.2f %.2f %.2f", para_Ex_Pose[0][0], para_Ex_Pose[0][1], para_Ex_Pose[0][2], para_Ex_Pose[0][3], para_Ex_Pose[0][4], para_Ex_Pose[0][5], para_Ex_Pose[0][6]);
        // Quaterniond qic{ric[i]};
        // ROS_INFO("IC: %.3f %.3f %.3f %.2f %.2f %.2f %.2f", tic[0].x(), tic[0].y(), tic[0].z(), qic.x(), qic.y(), qic.z(), qic.w());
    }
    ...
    // printf("para_angle: ");
    // for(int i = 0; i <= WINDOW_SIZE; i++)
    //     printf("%.3f ", para_angle[i][0]);
    // printf("\r\n");
    // printf("para_ce_Position: ");
    // for(int i = 0; i < SIZE_POSITION; i++)
    //     printf("%.3f ", para_ce_Position[0][SIZE_POSITION]);
    // printf("\r\n");
    // printf("para_Pose: ");
    // for(int i = 0; i <= WINDOW_SIZE; i++)
    //     printf("%.3f ", para_Pose[i][0]);
    // printf("\r\n");
...
void Estimator::double2vector()
{
    ...
    for (int i = 0; i <= WINDOW_SIZE; i++)
    {
        ...
        if(ENCODER_ENABLE)
            Encoder_angle[i][0] = para_angle[i][0];
    }
    for (int i = 0; i < NUM_OF_CAM; i++)
    {
        ...
        {
            // Eigen::Matrix4d pose = Eigen::Matrix4d::Identity();
            // pose.block<3, 3>(0, 0) = ric[i];
            // pose.block<3, 1>(0, 3) = tic[i];

            // double theta = -(4095 - last_encoder_data) / 4096.0 * 2 * M_PI;
            // Eigen::Matrix4d T = Eigen::Matrix4d::Identity();
            // T.block<3, 3>(0, 0) = Eigen::AngleAxisd(theta + M_PI, Eigen::Vector3d::UnitZ()).toRotationMatrix();
            // T = T.inverse() * pose;

            // RIE[i] = T.block<3, 3>(0, 0);
            // TIE[i] = T.block<3, 1>(0, 3);
...
{
    // ROS_INFO("marginalization_flag = %d", marginalization_flag);
    ...
    for (int i = 0; i < WINDOW_SIZE + 1; i++)
    {
        ...
        if (ENCODER_ENABLE)
        {
            ...
            problem.SetParameterLowerBound(para_angle[i], 0, Encoder_angle_original[i] - 0.001);
            problem.SetParameterUpperBound(para_angle[i], 0, Encoder_angle_original[i] + 0.001);
        }
    }
    for (int i = 0; i < NUM_OF_CAM; i++)
    {
    ...
    for (auto &it_per_id : f_manager.feature)
    {
        ...
        for (auto &it_per_frame : it_per_id.feature_per_frame)
        {
            ...
            else
            {
                ...
                {
                    ProjectionFactor *f = new ProjectionFactor(pts_i, pts_j, Encoder_angle[imu_i][0], Encoder_angle[imu_j][0]);
        ...
        for (auto &it_per_id : f_manager.feature)
        {
            ...
            {   
                ...
                if((int)match_points[retrive_feature_index].z() == it_per_id.feature_id)
                {
                    ...
                    {
                        ProjectionFactor *f = new ProjectionFactor(pts_i, pts_j, Encoder_angle[start][0], Encoder_angle[relo_frame_local_index][0]);
    ...
    if (marginalization_flag == MARGIN_OLD)
    {
        ...
        // cout << "addr_shift size: " << addr_shift.size() << endl;
...
void Estimator::slideWindow()
{
    ...
    if (marginalization_flag == MARGIN_OLD)
    {
        ...
        if (frame_count == WINDOW_SIZE)
        {
            for (int i = 0; i < WINDOW_SIZE; i++)
            {
                ...
                Bas[i].swap(Bas[i + 1]);
                Bgs[i].swap(Bgs[i + 1]);
                std::swap(Encoder_angle[i][0], Encoder_angle[i + 1][0]);
                std::swap(Encoder_angle_original[i], Encoder_angle_original[i + 1]);
                std::swap(ric[i], ric[i + 1]);
                std::swap(tic[i], tic[i + 1]);
            }
            ...
            Encoder_angle[WINDOW_SIZE][0] = Encoder_angle[WINDOW_SIZE - 1][0];
            Encoder_angle_original[WINDOW_SIZE] = Encoder_angle_original[WINDOW_SIZE - 1];
            ric[WINDOW_SIZE] = ric[WINDOW_SIZE - 1];
            tic[WINDOW_SIZE] = tic[WINDOW_SIZE - 1];
    ...
    else
    {
        if (frame_count == WINDOW_SIZE)
        {
            ...
            Encoder_angle[frame_count - 1][0] = Encoder_angle[frame_count][0];
            Encoder_angle_original[frame_count - 1] = Encoder_angle_original[frame_count];
            ric[frame_count - 1] = ric[frame_count];
            tic[frame_count - 1] = tic[frame_count];
